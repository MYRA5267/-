...
import { statusAfterEdit } from './status';
import { logAction, track } from './audit';
import { generatePack, rewriteVariant } from './ai/generate';
...
export async function generateForIdea(
  ...
  const context = await withUser(tgId, async (client) => {
    const { rows } = await client.query<IdeaRow & { project_name: string }>(
      `select c.id, c.project_id, c.title, c.source_text, c.objective, c.audience,
              c.status, c.idea_state, c.created_at, p.name as project_name
         from pulse.content_items c
    ...
    const { rows: existing } = await client.query<{ platform: Platform }>(
      'select platform from pulse.platform_variants where content_item_id = $1',
      [contentId],
    );
    return {
      idea: toIdea(rows[0]),
      brand,
      recentHooks,
      existing: new Set(existing.map((e) => e.platform)),
    };
  });
  ...
  await withUser(tgId, async (client) => {
    ...
    await client.query(
      `update pulse.content_items
          set status = case when status = 'IDEA' then 'DRAFT' else status end,
